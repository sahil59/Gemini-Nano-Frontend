...
const Rewrite = ({fileDetails}) => {
    ...
    const fetchStreamingWrittenText = async () => {
        try {
            const stream = await rewriteContent('History of Latin type The rich history of type design and the technical evolution of typesetting are woven into our everyday practice of typography, from how we select fonts to how we use spacing. And while the tools we have today are more capable than ever, designing is still about problem solving. Looking to the challenges and solutions from the past helps give us a deeper understanding of how type works.', 'write in bullet points');
            let accumulatedText = '';
            ...
            console.error('Error fetching streamed written text:', error);
        }
    };
    ...
    useEffect(() => {
        const runWriter = async () => {
            ...
            if (support) {
                ...
                const availableType = 'readily';
                if (availableType === 'readily') {
                    setAvailable(true);
                    setAvailableLoading(false);
                    fetchStreamingWrittenText();
                } else {
                    setAvailable(false);
                    setAvailableLoading(false);
                }
        ...
        <section className='flex flex-col items-start w-full gap-2'>
            <div className='flex items-start justify-start gap-2'>
                ...
                <h2 className='font-bold w-full text-black dark:text-white duration-300 ease-in-out'>
                    Rewrite content for the file - {fileDetails.name}
                </h2>
            </div>
                            ...
                            {support ?
                                <div className='flex gap-1 items-start'>
                                    ...
                                    <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                        Rewriter is supported in your browser.
                                    </span>
                                </div>
                                :
                                <div className='flex gap-1 items-start'>
                                    ...
                                    <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                        Rewriter is not supported in your browser.
                                    </span>
                                </div>
                            }
                        </div>
                            ...
                            <div>
                                {available ?
                                    <div className='flex gap-1 items-start'>
                                        ...
                                        <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                            Rewriter is available in your browser.
                                        </span>
                                    </div>
                                    :
                                    support ?
                                    <div className='flex gap-1 items-start'>
                                        ...
                                        <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                            Rewriter is supported but not available in your browser.
                                        </span>
                                    </div>
                                    :
                                    <div className='flex gap-1 items-start'>
                                        ...
                                        <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                            Rewriter is neither supported nor available in your browser.
                                        </span>
                                    </div>
                                }
                            </div>
