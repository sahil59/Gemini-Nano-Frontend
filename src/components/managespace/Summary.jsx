...
import { checkSummarySupport, getCapabilities, summarizeContent } from '../../functions/summarize';
import LoaderSpinner from '../reusable/LoaderSpinner';
import { useDispatch } from 'react-redux';
import { fileActionType } from '../../redux/action';
import PropTypes from 'prop-types';

const Summary = ({fileDetails}) => {
    ...
    const fetchStreamingSummary = async () => {
        try {
            ...
            console.error('Error fetching streamed summary:', error);
        }
    };

    useEffect(() => {
        const runSummary = async () => {
            const support = checkSummarySupport();
            if (support) {
                setSupport(true);
                setSupportLoading(false);
                const availableType = await getCapabilities();
        ...
        <section className='flex flex-col items-start w-full gap-2'>
            <div className='flex items-start justify-start gap-2'>
                ...
                <h2 className='font-bold w-full text-black dark:text-white duration-300 ease-in-out'>
                    Summarization of file - {fileDetails.name}
                </h2>
            </div>
                            ...
                            {support ?
                                <div className='flex gap-1 items-start'>
                                    ...
                                    <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                        Summarizer is supported in your browser.
                                    </span>
                                </div>
                                :
                                <div className='flex gap-1 items-start'>
                                    ...
                                    <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                        Summarizer is not supported in your browser.
                                    </span>
                                </div>
                            }
                        </div>
                            ...
                            <div>
                                {available ?
                                    <div className='flex gap-1 items-start'>
                                        ...
                                        <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                            Summarizer is available in your browser.
                                        </span>
                                    </div>
                                    :
                                    support ?
                                    <div className='flex gap-1 items-start'>
                                        ...
                                        <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                            Summarizer is supported but not available in your browser.
                                        </span>
                                    </div>
                                    :
                                    <div className='flex gap-1 items-start'>
                                        ...
                                        <span className='text-[0.8rem] text-[#4C4C4C] dark:text-[#c4c4c4] duration-300 ease-in-out'>
                                            Summarizer is neither supported nor available in your browser.
                                        </span>
                                    </div>
                                }
                            </div>
            ...
            <div className='text-black dark:text-white duration-300 ease-in-out text-[0.9rem]'>
                {streamedText}
